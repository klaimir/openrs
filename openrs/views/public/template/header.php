...
<?php }?>
  	<div class="container">
	    ...
	    <div class="menu-mv hidden-sm hidden-md hidden-lg col-xs-3">
			<div class="menu_bar">
				...
				<ul>
					...
					<?php if($seccion != 'admin' && $seccion != 'login' && $seccion !='mi_cuenta' && $seccion != 'error'){
						foreach ($secciones_header as $sh):
							...
							<?php if($sh->tipo_seccion == 1):?>
								<a href="<?php echo site_url($this->uri->segment('1').'/seccion/seccion/'.$sh->url_seo);?>" style="color:white"><?php echo $sh->titulo; ?></a>
							<?php elseif ($sh->tipo_seccion == 2): ?>
							...
							<?php elseif ($sh->tipo_seccion == 4): ?>
								<a href="<?php echo site_url($this->uri->segment('1').'/'.$sh->url_seo);?>" style="color:white"><?php echo $sh->titulo; ?></a>
							<?php elseif ($sh->tipo_seccion == 5): ?>
								<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false" style="color:white"><?php echo $sh->titulo; ?> <span class="caret"></span></a>
						          <ul class="dropdown-menu" role="menu">
							          <?php foreach($subsecciones_header as $ssh){?>
							            <li><a href="<?php echo site_url($ssh->url_seo);?>" style="color:white"><?php echo $ssh->titulo; ?></a></li>
							            <li class="divider"></li>
							          <?php }?>
						          </ul>
							<?php endif; ?>
					 ...
					 <?php }else{
						foreach ($secciones_header as $sh):?>
							<li>
							<?php if($sh->tipo_seccion == 1):?>
								<a href="<?php echo site_url('seccion/seccion/'.$sh->url_seo);?>" style="color:white"><?php echo $sh->titulo; ?></a>
							<?php elseif ($sh->tipo_seccion == 2): ?>
								<a href="<?php echo site_url($sh->url_seo);?>" style="color:<?php echo $config->cfuentecabecera;?>"><?php echo $sh->titulo; ?></a>
							<?php elseif ($sh->tipo_seccion == 3): ?>
								<a href="<?php echo site_url('blog-articulos');?>" style="color:<?php echo $config->cfuentecabecera;?>"><?php echo $sh->titulo; ?></a>
							<?php elseif ($sh->tipo_seccion == 4): ?>
								<a href="<?php echo site_url($sh->url_seo);?>" style="color:white"><?php echo $sh->titulo; ?></a>
							<?php elseif ($sh->tipo_seccion == 5): ?>
								<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false" style="color:white"><?php echo $sh->titulo; ?> <span class="caret"></span></a>
						          <ul class="dropdown-menu" role="menu">
							          <?php foreach($subsecciones_header as $ssh){?>
					...
					<?php }?>
					<?php if(count($cargar_idiomas) > 1){?>
					<li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" href="#" style="color:white"><?php echo strtoupper($idioma_actual->nombre_seo2);?> <b class="caret"></b></a>
						<ul class="dropdown-menu">		
							<?php foreach($cargar_idiomas as $cargar_idioma){?>
								<li><a tabindex="-1" data-id_actual="<?php echo $idioma_actual->id_idioma;?>" data-id="<?php echo $cargar_idioma->id_idioma;?>" class="cambio_idioma" style="cursor:pointer"><?php echo strtoupper($cargar_idioma->nombre_seo2);?></a> </li>
							<?php }?>			
