...
class Tipos_inmueble extends MY_Controller
{
    ...
    function general()
    {
        ...
        $result_validation_insert=$this->Tipo_inmueble_model->validation(); 
        $this->unit->run(FALSE, $result_validation_insert, 'Test de validación al insertar 1');

        $nombre2 = 'Nombre Inmueble';
        ...
        $this->form_validation->set_data($_POST);
        $result_validation_insert2=$this->Tipo_inmueble_model->validation(); 
        $this->unit->run(TRUE, $result_validation_insert2, 'Test de validación al insertar 2');
                    
        // Insert
        $datos_formateados = $this->Tipo_inmueble_model->get_formatted_datas();
        $id=$this->Tipo_inmueble_model->insert($datos_formateados);
        
        $datos_bd_insert=$this->Tipo_inmueble_model->get_by_id($id);
        ...
        $this->form_validation->reset_validation();
        $this->form_validation->set_data($_POST);
        $result_validation_edit=$this->Tipo_inmueble_model->validation($id);
        
        $this->unit->run(FALSE, $result_validation_edit, 'Test de validación al update 1');
        
        // Update
        $nombre_edit2 = 'Nombre Inmueble2';
        ...
        $this->form_validation->reset_validation();
        $this->form_validation->set_data($_POST);
        $result_validation_edit2=$this->Tipo_inmueble_model->validation($id);
        
        $this->unit->run(TRUE, $result_validation_edit2, 'Test de validación al update 2');
        
        $datos_formateados2 = $this->Tipo_inmueble_model->get_formatted_datas();
        $affected=$this->Tipo_inmueble_model->update($datos_formateados2,$id);
        
        $datos_bd_edit2=$this->Tipo_inmueble_model->get_by_id($id);
        ...
        // Delete        
        $result_validation_delete=$this->Tipo_inmueble_model->delete($id);
